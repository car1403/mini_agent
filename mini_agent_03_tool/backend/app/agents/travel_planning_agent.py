...
from app.tools.lab_tools import travel_data
...
def run_travel_planning_agent(session_id: str, message: str, arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    calls = [{"tool": weather_tool, "arguments": {"city": state["city"], "travel_date": target_date.isoformat()}}, {"tool": "search_attractions", "arguments": {"city": state["city"]}}]
    plan = travel_data(state["city"], target_date)
    trace.extend([{"stage": "agent_decision", "action": item["tool"]} for item in calls]); trace.append({"stage": "tool_result", "data": plan})
